...
class _ForcePasswordChangeScreenState extends State<ForcePasswordChangeScreen> {
  ...
  bool get _hasUpperLower =>
      RegExp(r'(?=.*[A-Z])(?=.*[a-z])').hasMatch(_newPasswordCtrl.text);
  bool get _hasNumber => RegExp(r'\d').hasMatch(_newPasswordCtrl.text);
  bool get _hasSpecial =>
      RegExp(r'[\!\@\#\$\%\^\&\*\.\,\?]').hasMatch(_newPasswordCtrl.text);
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      final dbUrl = Uri.parse(
          'https://crowdsense-db-default-rtdb.asia-southeast1.firebasedatabase.app/users/${user.uid}.json?auth=$idToken');
      ...
      if (dbResponse.statusCode != 200) {
        throw Exception(
            "Failed to sync status to database securely: ${dbResponse.statusCode}");
      }
      ...
      if (response.statusCode != 200) {
        throw Exception(
            "Failed to perform secure password exchange: ${response.statusCode}");
      }
            ...
            child: FadeInDown(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  color: AppColors
                      .surfaceDark, // CrowdSense Dark Theme Widget color
                            ...
                            decoration: InputDecoration(
                              ...
                              border: OutlineInputBorder(
                                ...
                                borderSide: _passwordsMatch
                                    ? const BorderSide(
                                        color: AppColors.statusSafe, width: 1.5)
                                    : BorderSide.none,
                              ),
                            ),
                          ),
                              ...
                              children: [
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      _buildCheckRow(
                                          "Min 8 characters", _hasMinLength),
                                      _buildCheckRow(
                                ...
                                Expanded(
                                  child: Column(
                                    crossAxisAlignment:
                                        CrossAxisAlignment.start,
                                    children: [
                                      _buildCheckRow("Uppercase & Lowercase",
                                          _hasUpperLower),
                                      _buildCheckRow(
                                          "Special Char (!@#\$%^&*.?)",
                                          _hasSpecial),
                        ...
                        child: ElevatedButton(
                          ...
                          style: ElevatedButton.styleFrom(
                            backgroundColor: AppColors
                                .statusDanger, // Orange/Red matching D1-IPAM submission button theme
